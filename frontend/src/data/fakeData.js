//exported as an object so you can reference the key in the dynamic resources routes. 
export default {
  scholarships: {
    photo: '/images/linkedin-sales-solutions-jJFrkJP_-RA-unsplash.jpg', 
    progams: [
      {
        name: 'Book Club', 
        href: 'book-club', 
        bio: 'Book club is a place all students can go to get their literature on. I\'ts where you go when you\'re unsure of what to do, why not escape to far off destination or somewhere else in the cosmos.', 
        photo: '/terry-tran-EG9GnUX3Wkw-unsplash.jpg'
      }, 
      {
        name: 'Mission Bit', 
        href: 'mission-bit', 
        bio: 'Helping students get ahead by giving them the knowledge in modern programming practices and algorithmic thinking', 
        overview: 'Mission Bit programming is here to help students excel, giving them a boost in critical thinking through practice and repition of building incredible small projects. There are several languages out there, so we teach the underlying fundamentals so that students can feel comfortable picking up any language from C to Javascript.', 
        photo: '/juanjo-jaramillo-mZnx9429i94-unsplash.jpg'
      }, 
      {
        name: 'Youth Mentorship', 
        href: 'mentorship', 
        bio: 'Help the younger kids coming up get ahead by giving them guidance. Only you can provide your experience, and help them get ahead before they even know they need it. '
      }
    ], 

  },
  summer: {
    photo: '/images/airfocus-f2C59x5uvn8-unsplash.jpg',
    programs: [
      {
        name: 'Book Club', 
        href: 'book-club', 
        bio: 'Book club is a place all students can go to get their literature on. I\'ts where you go when you\'re unsure of what to do, why not escape to far off destination or somewhere else in the cosmos.', 
        photo: '/terry-tran-EG9GnUX3Wkw-unsplash.jpg'
      }, 
      {
        name: 'Mission Bit', 
        href: 'mission-bit', 
        bio: 'Helping students get ahead by giving them the knowledge in modern programming practices and algorithmic thinking', 
        overview: 'Mission Bit programming is here to help students excel, giving them a boost in critical thinking through practice and repition of building incredible small projects. There are several languages out there, so we teach the underlying fundamentals so that students can feel comfortable picking up any language from C to Javascript.', 
        photo: '/juanjo-jaramillo-mZnx9429i94-unsplash.jpg'
      }, 
      {
        name: 'Youth Mentorship', 
        href: 'mentorship', 
        bio: 'Help the younger kids coming up get ahead by giving them guidance. Only you can provide your experience, and help them get ahead before they even know they need it. '
      }
    ], 
  },
  programs: {
    photo: '/images/heylagostechie-IgUR1iX0mqM-unsplash.jpg',
    programs: [
      {
        name: 'Book Club', 
        href: 'book-club', 
        bio: 'Book club is a place all students can go to get their literature on. I\'ts where you go when you\'re unsure of what to do, why not escape to far off destination or somewhere else in the cosmos.', 
        photo: '/terry-tran-EG9GnUX3Wkw-unsplash.jpg'
      }, 
      {
        name: 'Mission Bit', 
        href: 'mission-bit', 
        bio: 'Helping students get ahead by giving them the knowledge in modern programming practices and algorithmic thinking', 
        overview: 'Mission Bit programming is here to help students excel, giving them a boost in critical thinking through practice and repition of building incredible small projects. There are several languages out there, so we teach the underlying fundamentals so that students can feel comfortable picking up any language from C to Javascript.', 
        photo: '/juanjo-jaramillo-mZnx9429i94-unsplash.jpg'
      }, 
      {
        name: 'Youth Mentorship', 
        href: 'mentorship', 
        bio: 'Help the younger kids coming up get ahead by giving them guidance. Only you can provide your experience, and help them get ahead before they even know they need it. '
      }
    ], 
  },
  internships: {
    photo: '/images/israel-andrade-YI_9SivVt_s-unsplash.jpg',
    programs: [
      {
        name: 'Book Club', 
        href: 'book-club', 
        bio: 'Book club is a place all students can go to get their literature on. I\'ts where you go when you\'re unsure of what to do, why not escape to far off destination or somewhere else in the cosmos.', 
        photo: '/terry-tran-EG9GnUX3Wkw-unsplash.jpg'
      }, 
      {
        name: 'Mission Bit', 
        href: 'mission-bit', 
        bio: 'Helping students get ahead by giving them the knowledge in modern programming practices and algorithmic thinking', 
        overview: 'Mission Bit programming is here to help students excel, giving them a boost in critical thinking through practice and repition of building incredible small projects. There are several languages out there, so we teach the underlying fundamentals so that students can feel comfortable picking up any language from C to Javascript.', 
        photo: '/juanjo-jaramillo-mZnx9429i94-unsplash.jpg'
      }, 
      {
        name: 'Youth Mentorship', 
        href: 'mentorship', 
        bio: 'Help the younger kids coming up get ahead by giving them guidance. Only you can provide your experience, and help them get ahead before they even know they need it. '
      }
    ], 
  }
}